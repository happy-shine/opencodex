...
describe("CodexEngineAdapter", () => {
  ...
  it("writes the prompt to stdin instead of exposing it in argv", async () => {
    const script = createBinary(`
      let body = "";
      process.stdin.setEncoding("utf8");
      process.stdin.on("data", (chunk) => { body += chunk; });
      process.stdin.on("end", () => {
        const argvHasPrompt = process.argv.some((arg) => arg.includes("secret prompt"));
        const argvHasStdinMarker = process.argv.includes("-");
        console.log(JSON.stringify({
          type: "item.completed",
          item: { type: "agent_message", text: body + "|" + argvHasPrompt + "|" + argvHasStdinMarker },
        }));
        console.log(JSON.stringify({ type: "turn.completed", result: "done" }));
      });
    `);
    ...
    const events = await collect(adapter.sendMessage(createSession(), "secret prompt", "bot"));

    const textEvent = events.find((event): event is { type: "text"; text: string } => event.type === "text");
    expect(textEvent?.text).toContain("<opencodex-system>");
